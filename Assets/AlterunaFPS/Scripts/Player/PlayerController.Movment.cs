...
namespace AlterunaFPS
{
	public partial class PlayerController
	{
        ...
        private void Move()
        {
            ...
            if (_isClimbing)
            {
                // Движение по лестнице: W/S - вверх/вниз, A/D - вбок
                Vector3 climbMove = new Vector3(horizontal * ClimbSpeed * 0.5f, vertical * ClimbSpeed, 0);
                // Преобразуем в мировые координаты относительно направления лестницы
                climbMove = transform.TransformDirection(climbMove);
                _controller.Move(climbMove * Time.deltaTime);

                // Отключаем гравитацию пока на лестнице
                _verticalVelocity = 0;

                // Анимация
                if (_hasAnimator)
                {
                    _animator.SetFloat(_animIDSpeed, Mathf.Abs(vertical) * ClimbSpeed);
                    _animator.SetFloat(_animIDMotionSpeed, 1);
                }

                // Выход с лестницы по прыжку
                if (_jump)
                {
                    _isClimbing = false;
                    _verticalVelocity = Mathf.Sqrt(JumpHeight * -2f * Gravity); // Прыжок от лестницы
                }

                return; // Пропускаем обычное движение
            }

            // --- ОБЫЧНОЕ ДВИЖЕНИЕ (твой текущий код без изменений) ---
            float targetSpeed = 0;
            float inputMagnitude = 1f;
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            {
                _isClimbing = true;
                // Запоминаем направление лестницы (чтобы игрок смотрел на неё)
                _ladderForward = -other.transform.forward;
            }
        }
